...
export function Profile(){
    ...
    const changeAvatar = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
        if(event.target.files){
            if (user.token === '') {
                alert(
                    'Для загрузки фото необходимо авторизоваться.'
                );
            }
            ...
            uploadAvatar(event.target.files[0], user.token).then(res => {
    ...
    },[difficultWordChange, deletedWordChange, translateWordChange, translateSentencesChange, musicVolumeChange, soundVolumeChange, wordVolumeChange, dispatch, user]);
        ...
        <div className="profile">
               <div className="profile__top">
                    <Box className='profile-item' sx={{
                        ...
                        <div className="settings">
                            ...
                            <div className="settings-btn__container">
                                <p>Сложные слова</p>
                                <Switch onClick={() => {setDifficultWordChange(!difficultWordChange)}} checked={difficultWordChange} />
                            </div>
                            <div className="settings-btn__container">
                                <p>Удалить Слово</p>
                    ...
                    <Box className='profile-item' sx={{
                        ...
                        <div className="settings">
                            <Avatar className="avatar" src={user.avatarURL} sx={{
                                height: 100,
                                width: 100,
                                margin: '20px auto',
                            }}/>
                            ...
                            <Button variant="contained" component="label" onClick={() => handleChangeName()}>Сменить никнейм</Button>
                            <p className='current-name'>Текущий никнейм:</p>
                            <p>{user.userName}</p>
                            <Button variant="contained" component="label" onClick={() => handleExit()}>Выход</Button>
                        </div>
                    </Box>
                    <Box className='profile-item' sx={{
                        width: 300,
                        ...
                        <h2>Связь с нами</h2>
                        <p>Ваш email</p>
                        <TextField id="filled-basic" label="Email" variant="filled" margin='normal' color='secondary' onChange={(e) => setFeedbackEmail(e.target.value)}/>
                        <p>Ваше сообщение</p>
                        <TextField id="filled-basic" fullWidth label='Ваше сообщение' margin='normal' color='secondary' onChange={(e) => setFeedbackMessage(e.currentTarget.value)}/>
                        </div>
                        <Button variant="contained" component="label" onClick={() => postFeedback(feedbackMessage, feedbackEmail)} >Отправить</Button>
                    </Box>
                    
               </div>
               <Button className='settings-accept__btn' variant="contained" component="label" onClick={() => handleSendSettings()}><p>Преминить настройки</p></Button>
